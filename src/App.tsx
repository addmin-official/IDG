...
import { 
  ...
  TrendingUp, Globe, FileText, Send, HelpCircle, Server, 
  MapPin, RefreshCw, LayoutGrid, Layers, Landmark, Network, 
  ArrowRight, CornerDownRight, CheckSquare, ChevronRight, BookOpen
} from 'lucide-react';
import { CHECKPOINTS, SYSTEM_SERVICES, TRADE_ALERTS, CARGO_PRESETS, DICTIONARY, MOCK_LEDGER_BLOCKS } from './mockData';
import { Language, Checkpoint, TradeAlert, CargoManifest, SystemService, AIAnalysisResult } from './types';
import EcosystemWorkflows from './components/EcosystemWorkflows';
import SovereignAIBrain from './components/SovereignAIBrain';
...
export default function App() {
  ...
  // Tab State
  // 'command-center' | 'blueprints' | 'ai-auditor' | 'policy-advisor' | 'economic-corridors'
  const [activeTab, setActiveTab] = useState<string>('command-center');

  // Command Center Sub-states
  const [selectedCheckpoint, setSelectedCheckpoint] = useState<Checkpoint>(CHECKPOINTS[0]);
  const [ledgerBlocks, setLedgerBlocks] = useState(MOCK_LEDGER_BLOCKS);
  const [alerts, setAlerts] = useState<TradeAlert[]>(TRADE_ALERTS);

  // AI Auditor Sub-states
  ...
  // System general heartbeat simulator info
  ...
  const [revenueTodayCumulative, setRevenueTodayCumulative] = useState<number>(31940); // in millions IQD

  // Core Active Blueprint selected in Architectural Atlas
  const [activeBlueprintId, setActiveBlueprintId] = useState<string>('system-context');

  // Update cumulative numbers slightly in background to make command center feel alive
  useEffect(() => {
  ...
  // Load preset details into editable fields
  ...
  // Submit manifest to server for Gemini AI processing
  ...
  const handleSendChat = async (typedText?: string) => {
    ...
    try {
      ...
      setChatHistory(prev => [...prev, { 
        sender: 'system', 
        text: 'The Federal Knowledge core experienced offline failover. Baghdad-Erbil unified treaty instructions and standard tarrif guides remain loaded on-cache.' 
      }]);
    } finally {
  ...
  // Trigger macro forecasting simulation
  const handleTriggerForecast = async () => {
    ...
    try {
      const response = await fetch('/api/idg/generate-forecast', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ corridorId: selectedCorridor, policyLevel, cbiMode: cbiSurveillance })
      });
      const data = await response.json();
  ...
  const BLUEPRINT_METADATA = [
    {
      ...
      desc: 'The global interaction map depicting the IDG core as Iraq\'s unified trade border gateway interfaces with sovereign state agents and international merchants.',
      points: [
    ...
    },
    {
      ...
      title: '2. Business Capability Map',
      desc: 'Operational capability taxonomy defining the core of our sovereign trade infrastructure. Designed to build trust and eradicate custom duty evasion.',
      points: [
    ...
    {
      id: 'ddd-boundaries',
      title: '3. Domain Driven Design Boundaries',
      desc: 'Sovereign domain bounded contexts separated with strict transaction contracts to support modularity, hot-swappability, and resilient local isolation.',
      points: [
    ...
    },
    {
      ...
      title: '4. Event-Driven Messaging Architecture',
      desc: 'The asynchronous publish-subscribe workflow handling heavy national logistics loads. Guarantees 100% offline edge processing resilience.',
      points: [
    ...
    },
    {
      ...
      title: '5. Horizontal Microservice Architecture',
      desc: 'Stateless custom microservices scaling horizontally on containerized Kubernetes clusters. Free from single-point-of-failure vulnerabilities.',
      points: [
    ...
    },
    {
      ...
      title: '6. Zero-Trust Security Architecture',
      desc: 'The federal-grade posture protecting national trade streams against cyber threats, tampering, in-transit evasion, and fraud.',
      points: [
    ...
    },
    {
      ...
      title: '7. Data Architecture & Master Registries',
      desc: 'The unified multi-region hybrid database structure maintaining ledger compliance and national trade history.',
      points: [
    ...
    },
    {
      ...
      title: '8. AI Cognitive Architecture (Gemini Engine)',
      desc: 'Deep cognitive processing stack utilizing Gemini-3.5-flash to analyze physical manifest parameters against federal law datasets.',
      points: [
    ...
    },
    {
      ...
      title: '9. Unified Government Web Bridge',
      desc: 'Inter-ministerial integration hub providing standard Restful, gRPC, and SOAP adapters connecting legacy and modern state systems.',
      points: [
    ...
    },
    {
      ...
      title: '10. National Command Center Topology',
      desc: 'Centralized telemetry, latency monitoring, real-time alert aggregation, and emergency country-wide bypass failovers.',
      points: [
          ...
          <div className="bg-[#111e2e]/90 p-5 lg:p-6 rounded-xl border border-slate-800/80 shadow-lg animate-fade-in flex flex-col gap-6">
            ...
            <div className="border-b border-slate-800 pb-4">
              <h2 className="text-lg font-display font-semibold text-slate-100 flex items-center gap-2">
                ...
                Sovereign Digital Infrastructure - Master Architect Blueprint Suite
              </h2>
              <p className="text-slate-400 text-xs mt-1">
                Explore the formal enterprise-grade system diagrams prepared by the Chief enterprise and systems architects for the Republic of Iraq Trade & Customs Ecosystem.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              ...
              <div className="lg:col-span-1 flex flex-col gap-1">
                <h3 className="text-xs uppercase tracking-wider text-slate-400 font-mono mb-2 px-2 font-bold">Cabinet Blueprints (A-Z)</h3>
              ...
              <div className="lg:col-span-3 bg-slate-950/85 rounded-xl border border-slate-800/80 p-5 md:p-6 flex flex-col gap-6">
                ...
                <div>
                  <h3 className="text-base font-display font-semibold text-[#cca553] flex items-center gap-2">
                    ...
                    {BLUEPRINT_METADATA.find(b => b.id === activeBlueprintId)?.title}
                  </h3>
                  <p className="text-slate-300 text-xs leading-relaxed mt-2 p-3 bg-slate-900/60 rounded border-l-2 border-[#cca553]/60 italic font-mono">
                    "{BLUEPRINT_METADATA.find(b => b.id === activeBlueprintId)?.desc}"
                  </p>
                </div>
                ...
                <div className="bg-[#0c1421] rounded-xl p-4 md:p-6 border border-slate-800 shadow-inner flex items-center justify-center min-h-[300px]">
                  ...
                  {activeBlueprintId === 'system-context' && (
                    <svg viewBox="0 0 800 400" className="w-full max-w-[650px] h-auto font-mono text-[10px] text-slate-300">
                      ...
                      <g transform="translate(300, 150)">
                        <rect x="0" y="0" width="200" height="100" rx="6" fill="#111e2e" stroke="#cca553" strokeWidth="2" className="glow-embed"/>
                      ...
                      {/* CONNECTIONS & LINES (Animated / Glowing arrows simulation) */}
                      {/* Importers -> IDG Core */}
                      ...
                      <text x="235" y="115" angle="-45" fill="#94a3b8" fontSize="8">Secure REST API</text>
                  ...
                  {activeBlueprintId === 'event-architecture' && (
                    <svg viewBox="0 0 800 400" className="w-full max-w-[650px] h-auto font-mono text-[10px] text-slate-300">
                      ...
                      <rect x="50" y="180" width="700" height="40" rx="4" fill="#111e2e" stroke="#cca553" strokeWidth="2" className="glow-embed"/>
                  ...
                  {/* Dynamic fallback details text description for remaining blueprints */}
                  {!['system-context', 'business-capabilities', 'ddd-boundaries', 'event-architecture'].includes(activeBlueprintId) && (
                    <div className="text-center py-6 px-4">
                      <div className="w-16 h-16 rounded-full bg-[#111e2e] border border-[#cca553] flex items-center justify-center mx-auto mb-4 shadow glow-embed">
                        <Server className="w-7 h-7 text-[#cca553]" />
                      </div>
                      <h4 className="font-semibold text-slate-200 uppercase tracking-widest text-sm mb-1">
                        Sovereign Specification Detail
                      </h4>
                      <p className="text-xs text-slate-400 max-w-md mx-auto mb-4">
                        Full implementation blueprints, API specifications, database parameters, and server configurations are optimized as decoupled service structures below.
                      </p>
                      <div className="inline-block bg-[#1a2c42] border border-slate-700/60 rounded px-3 py-1.5 text-xs text-slate-300 font-mono">
                        Security Clearance Certified • Zero-Trust Interoperability Handled
                      </div>
                    </div>
                  )}
                ...
                <div className="mt-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {BLUEPRINT_METADATA.find(b => b.id === activeBlueprintId)?.points.map((pt, idx) => (
              ...
              <div className="mb-5 bg-slate-950/60 p-4 rounded-lg border border-slate-800">
                ...
                <div className="flex flex-col gap-2">
                  {CARGO_PRESETS.map((preset) => (
                    <button
                      key={preset.manifestId}
                      onClick={() => handlePresetSelect(preset.manifestId)}
                      className={`w-full text-left p-2.5 rounded text-xs transition-all border flex flex-col ${
                        selectedPreset === preset.manifestId 
                          ? 'bg-[#1a2c42] border-[#cca553] text-white shadow' 
                          : 'bg-[#101925] border-slate-800 text-slate-400 hover:text-slate-200'
                      }`}
                    >
                      <div className="flex justify-between items-center w-full">
                        <span className="font-mono font-semibold text-slate-300 text-[11px]">{preset.manifestId}</span>
                        <span className="text-[10px] text-slate-500 italic font-mono">{preset.originCountry}</span>
                      </div>
                      <span className="truncate max-w-[240px] text-slate-400 font-medium text-[11px] mt-1">{preset.importerName}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 font-mono text-xs">
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    rows={3}
                    className="w-full bg-slate-950/80 border border-slate-800 rounded p-2 text-slate-200 focus:outline-none focus:border-[#cca553] text-xs resize-none"
                  />
                </div>
        ...
        {activeTab === 'policy-advisor' && (
          <div className="max-w-4xl mx-auto bg-[#111e2e]/95 rounded-xl border border-slate-800/80 shadow-lg p-5 lg:p-6 flex flex-col gap-4 animate-fade-in">
            ...
            <div className="border-b border-slate-800 pb-3 mb-2 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
              ...
              <span className="bg-[#cca553]/10 text-[#cca553] border border-[#cca553]/20 px-2.5 py-1 rounded text-xs font-mono uppercase tracking-wide">
                Authorized Official
              </span>
            </div>
